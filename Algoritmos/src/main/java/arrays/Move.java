...
public class Move {
     ...
     * @return the array with the new positions
     */
    public static <T extends Comparable<? super T>> T[] shiftItemsRight(T[] arr, int startPosition){
        for (int i = arr.length - 2; startPosition <= i; i--){
            arr[i + 1] = arr[i];
        }
        return arr;
     ...
     * @return the array with the new positions
     */
    public static <T extends Comparable<? super T>> T[] shiftItemsLeft(T[] arr, int startPosition){
        for (int i = startPosition + 1; i < arr.length; i++){
            arr[i - 1] = arr[i];
        }
        return arr;
